...
install_dependencies() {
    ...
    # Preguntar por herramientas adicionales de desarrollo backend
    echo
    info "¿Deseas instalar herramientas adicionales para desarrollo backend?"
    
    # Docker
    if confirm "¿Instalar Docker para contenedores?"; then
        install_docker
    fi
    
    # Node.js
    if confirm "¿Instalar Node.js y npm para desarrollo JavaScript/TypeScript?"; then
        install_nodejs
    fi
    
    # PHP
    if confirm "¿Instalar PHP para desarrollo web backend?"; then
        install_php
    fi
    
    # Postman/Insomnia
    if confirm "¿Instalar herramientas de testing de APIs (Postman/Insomnia)?"; then
        install_postman_insomnia
    fi
    
    success "Instalación de dependencias y herramientas completada"
}
...
show_menu() {
    ...
    printf "%-42s %s\n" "10) Instalar Spotify" "21) Instalar Docker"
    printf "%-42s %s\n" "11) Instalar VLC" "22) Usar Docker sin sudo"
    printf "%-42s %s\n" "23) Instalar Postman/Insomnia" "24) Instalar Node.js y npm"
    printf "%-42s %s\n" "25) Instalar PHP" "0)  Salir"
    echo
...
install_postman_insomnia() {
    info "Instalando Postman y/o Insomnia..."
    ...
    case $DISTRO_FAMILY in
        debian)
            if [[ "$choice" == "1" || "$choice" == "3" ]]; then
                ...
                success "Postman instalado. Ejecuta 'postman' para abrir."
            fi
            if [[ "$choice" == "2" || "$choice" == "3" ]]; then
                info "Instalando Insomnia..."
                wget -O /tmp/insomnia.deb "https://updates.insomnia.rest/downloads/ubuntu/latest?&app=com.insomnia.app&source=website"
        ...
        rpm)
            ...
            if [[ "$choice" == "2" || "$choice" == "3" ]]; then
                info "Instalando Insomnia..."
                wget -O /tmp/insomnia.rpm "https://updates.insomnia.rest/downloads/rpm/latest?&app=com.insomnia.app&source=website"
                sudo dnf install -y /tmp/insomnia.rpm || sudo yum install -y /tmp/insomnia.rpm
                success "Insomnia instalado."
            fi
            ;;
        arch)
            if [[ "$choice" == "1" || "$choice" == "3" ]]; then
                info "Instalando Postman desde AUR..."
                if command -v yay &> /dev/null; then
                    ...
                    cd - > /dev/null && rm -rf /tmp/yay-git
                    yay -S --noconfirm postman-bin
                fi
                success "Postman instalado."
            fi
            if [[ "$choice" == "2" || "$choice" == "3" ]]; then
                info "Instalando Insomnia desde AUR..."
                if command -v yay &> /dev/null; then
            ...
            ;;
        *)
            error "Familia de distribución no soportada para esta instalación."
            ;;
    esac
} 
install_docker() {
    ...
    case $DISTRO_FAMILY in
        debian)
            ...
            success "Docker instalado correctamente en Debian/Ubuntu."
            ;;
        rpm)
            ...
            sudo systemctl enable docker
            sudo systemctl start docker
            success "Docker instalado correctamente en Fedora/RHEL/CentOS."
            ;;
        arch)
            sudo pacman -Sy --noconfirm docker
            sudo systemctl enable docker
            sudo systemctl start docker
            success "Docker instalado correctamente en Arch/Manjaro."
            ;;
        *)
            error "Familia de distribución no soportada para instalación automática de Docker."
            ;;
    esac
}
docker_without_sudo() {
    info "Configurando Docker para usarse sin sudo..."
    read -p "Ingresa el nombre de usuario a añadir al grupo 'docker' (Enter para usar el usuario actual: $USER): " docker_user
    if [ -z "$docker_user" ]; then
        docker_user="$USER"
    fi
    sudo groupadd docker 2>/dev/null || true
    sudo usermod -aG docker "$docker_user"
    success "El usuario '$docker_user' ha sido añadido al grupo 'docker'."
    warning "Debes cerrar sesión y volver a iniciarla para que los cambios tengan efecto."
}

# Instalar Node.js y npm con NVM
install_nodejs() {
    info "Instalando Node.js y npm con NVM..."
    ...
    echo "¿Qué versión de Node.js deseas instalar?"
    echo "1) Node.js 20 LTS (recomendado)"
    echo "2) Node.js 22 (última versión)"
    echo "3) Ambas versiones"
    read -p "Selecciona (1-3): " node_choice
    ...
    case $node_choice in
        1)
            ...
            success "Node.js 20 LTS instalado y configurado como predeterminado"
            ;;
        2)
            ...
            success "Node.js 22 instalado y configurado como predeterminado"
            ;;
        3)
            info "Instalando ambas versiones..."
            nvm install 20
            nvm install 22
            nvm use 20
            nvm alias default 20
            success "Node.js 20 y 22 instalados. Versión predeterminada: 20 LTS"
            info "Para cambiar de versión usa: 'nvm use 20' o 'nvm use 22'"
            ;;
        *)
            error "Opción inválida"
            return 1
    ...
    info "Versiones de Node.js disponibles:"
    nvm list
    
    # Mostrar información de las herramientas instaladas
    ...
    echo "NVM: $(nvm --version 2>/dev/null || echo 'No disponible')"
    echo
    info "Para usar NVM en nuevas sesiones de terminal, reinicia tu terminal o ejecuta:"
    echo "source ~/.bashrc"
    echo
    info "Comandos útiles de NVM:"
    echo "nvm list                 - Ver versiones instaladas"
    echo "nvm install <version>    - Instalar una versión"
    echo "nvm use <version>        - Cambiar a una versión"
    echo "nvm alias default <version> - Establecer versión predeterminada"
}

# Instalar PHP con múltiples versiones
install_php() {
    info "Instalando PHP..."
    
    echo "¿Qué versión(es) de PHP deseas instalar?"
    echo "1) PHP 8.4 (última versión)"
    echo "2) PHP 8.2 (LTS recomendado)"
    echo "3) PHP 7.4 (para proyectos legacy)"
    echo "4) PHP 8.4 + 7.4 (proyectos nuevos y antiguos)"
    echo "5) Todas las versiones (8.4, 8.2, 7.4)"
    read -p "Selecciona (1-5): " php_choice
    
    case $DISTRO_FAMILY in
        debian)
            # Agregar repositorio ondrej/php para versiones múltiples
            ...
            case $php_choice in
                1)
                    info "Instalando PHP 8.4..."
                    sudo apt install -y php8.4 php8.4-cli php8.4-common php8.4-curl php8.4-mbstring php8.4-mysql php8.4-xml php8.4-zip php8.4-gd php8.4-intl php8.4-bcmath
                    sudo update-alternatives --set php /usr/bin/php8.4
                    success "PHP 8.4 instalado y configurado como predeterminado"
                    ;;
                2)
                    info "Instalando PHP 8.2..."
                    sudo apt install -y php8.2 php8.2-cli php8.2-common php8.2-curl php8.2-mbstring php8.2-mysql php8.2-xml php8.2-zip php8.2-gd php8.2-intl php8.2-bcmath
                    sudo update-alternatives --set php /usr/bin/php8.2
                    success "PHP 8.2 instalado y configurado como predeterminado"
                    ;;
                3)
                    info "Instalando PHP 7.4..."
                    sudo apt install -y php7.4 php7.4-cli php7.4-common php7.4-curl php7.4-mbstring php7.4-mysql php7.4-xml php7.4-zip php7.4-gd php7.4-intl php7.4-bcmath
                    sudo update-alternatives --set php /usr/bin/php7.4
                    success "PHP 7.4 instalado y configurado como predeterminado"
                    ;;
                4)
                    info "Instalando PHP 8.4 y 7.4..."
                    sudo apt install -y php8.4 php8.4-cli php8.4-common php8.4-curl php8.4-mbstring php8.4-mysql php8.4-xml php8.4-zip php8.4-gd php8.4-intl php8.4-bcmath
                    sudo apt install -y php7.4 php7.4-cli php7.4-common php7.4-curl php7.4-mbstring php7.4-mysql php7.4-xml php7.4-zip php7.4-gd php7.4-intl php7.4-bcmath
                    sudo update-alternatives --set php /usr/bin/php8.4
                    success "PHP 8.4 y 7.4 instalados. Predeterminado: 8.4"
                    ;;
                5)
                    info "Instalando todas las versiones de PHP..."
                    sudo apt install -y php8.4 php8.4-cli php8.4-common php8.4-curl php8.4-mbstring php8.4-mysql php8.4-xml php8.4-zip php8.4-gd php8.4-intl php8.4-bcmath
                    sudo apt install -y php8.2 php8.2-cli php8.2-common php8.2-curl php8.2-mbstring php8.2-mysql php8.2-xml php8.2-zip php8.2-gd php8.2-intl php8.2-bcmath
                    sudo apt install -y php7.4 php7.4-cli php7.4-common php7.4-curl php7.4-mbstring php7.4-mysql php7.4-xml php7.4-zip php7.4-gd php7.4-intl php7.4-bcmath
                    sudo update-alternatives --set php /usr/bin/php8.2
                    success "Todas las versiones de PHP instaladas. Predeterminado: 8.2 LTS"
                    ;;
                *)
                    error "Opción inválida"
                    return 1
            ...
            ;;
        rpm)
            info "Instalando PHP en sistemas RPM..."
            ...
            case $php_choice in
                1)
                    ...
                    sudo dnf install -y php php-cli php-common php-curl php-mbstring php-mysqlnd php-xml php-zip php-gd php-intl php-bcmath
                    success "PHP 8.4 instalado"
                    ;;
                2)
                    sudo dnf module reset php -y
                ...
                3)
                    ...
                    sudo dnf install -y php php-cli php-common php-curl php-mbstring php-mysqlnd php-xml php-zip php-gd php-intl php-bcmath
                    success "PHP 7.4 instalado"
                    ;;
                *)
                    warning "Para múltiples versiones en RPM, instala manualmente cada versión"
                    sudo dnf module enable php:remi-8.2 -y
                    sudo dnf install -y php php-cli php-common php-curl php-mbstring php-mysqlnd php-xml php-zip php-gd php-intl php-bcmath
                    success "PHP 8.2 instalado como predeterminado"
                    ;;
            esac
        ...
        arch)
            case $php_choice in
                1)
                    sudo pacman -S --noconfirm php php-gd php-intl php-curl php-mysql
                    success "PHP (última versión) instalado"
                    ;;
                2|*)
                    sudo pacman -S --noconfirm php php-gd php-intl php-curl php-mysql
                    ...
                    ;;
                3)
                    warning "PHP 7.4 no está disponible en repositorios oficiales de Arch"
                    info "Instalando PHP actual..."
                    sudo pacman -S --noconfirm php php-gd php-intl php-curl php-mysql
                    success "PHP instalado"
                    ;;
            esac
            ...
            ;;
        *)
            error "Familia de distribución no soportada para instalación automática de PHP."
            return 1
            ;;
    esac
    
    # Mostrar información de las versiones instaladas
    echo
    echo "Información de PHP instalado:"
    php_version=$(php --version 2>/dev/null | head -n 1 || echo "No disponible")
    composer_version=$(composer --version 2>/dev/null | head -n 1 || echo "No disponible")
    ...
    if [[ "$DISTRO_FAMILY" == "debian" ]]; then
        echo
        info "Versiones de PHP disponibles:"
        ls /usr/bin/php* 2>/dev/null | grep -E "php[0-9]\.[0-9]$" || echo "Solo versión predeterminada"
        echo
        info "Para cambiar entre versiones de PHP (solo Debian/Ubuntu):"
        echo "sudo update-alternatives --config php"
        echo "O directamente: sudo update-alternatives --set php /usr/bin/php8.4"
    fi
    
    echo
    info "Extensiones PHP comunes instaladas:"
    echo "- curl, mbstring, mysql, xml, zip, gd, intl, bcmath"
    echo
    info "Para verificar extensiones instaladas:"
    echo "php -m"
}
